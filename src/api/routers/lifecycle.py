...
class ProductDetectRequest(BaseModel):
    ...
    # 历史ROI数据（用于趋势判断）
    # 近N天的每日ROI，按日期倒序排列
    recent_roi_history: list[float] = []  # e.g., [0.52, 0.48, 0.55] 表示最近3天
...
async def detect_product_lifecycle(req: ProductDetectRequest):
    ...
    基于ROI的判定逻辑:
    - observing: 投放 < 3天
    - entry: 近3天ROI均值 > 40%（入场信号）
    - sustained: 近5天ROI在30%-80%波动（稳定信号）
    - growth: ROI > 40% 且趋势上升
    - decline: ROI持续下滑 > 30%
    - exit: ROI < 10% 持续5天
    - dead: 无投放
    ...
    result = product_detector.detect(
        ...
        order_amt=req.order_amt,
        ad_amt=req.ad_amt,
        recent_roi_history=req.recent_roi_history,
    )
...
async def list_stages(dimension: str = Query(..., description="维度: product 或 campaign")):
    ...
    # Campaign阶段（基于ROI）
    campaign_stages = [
        {"value": "campaign_observing", "label": "待观察", "description": "投放 < 24h，时间不足无法判断"},
        {"value": "campaign_cold_dead", "label": "冷死亡", "description": "投放 > 72h且从未产生收入"},
    ...
    # Product阶段（基于回测数据）
    product_stages = [
        {"value": "product_observing", "label": "待观察", "description": "投放 < 3天，时间不足"},
        {"value": "product_entry", "label": "入场期", "description": "近3天ROI均值 > 40%，入场信号"},
        {"value": "product_sustained", "label": "稳定期", "description": "近5天ROI均在30%-80%之间"},
